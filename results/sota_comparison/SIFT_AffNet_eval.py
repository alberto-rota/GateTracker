...
# -*- coding: utf-8 -*-
"""
SIFT + AffNet Evaluation Script

This script evaluates SIFT feature detection with AffNet descriptors and AdaLAM matching
...
from kornia_moons.feature import OpenCVDetectorKornia, OpenCVDetectorWithAffNetKornia
...
def initialize_models(device, max_epipolar_distance=1.0):
    """Initialize SIFT detector, AffNet descriptor, AdaLAM matcher, and RANSAC."""
    # Create OpenCV SIFT detector with Kornia wrapper
    sift_detector = OpenCVDetectorWithAffNetKornia(cv2.SIFT_create(8000))
    
    # Create AffNet descriptor
    descriptor = KF.HardNet8(pretrained=True).eval().to(device)
    
    # Combine detector and descriptor using LocalFeature
    local_feature = KF.LocalFeature(sift_detector, KF.LAFDescriptor(descriptor)).to(device)
...
def extract_and_match_features(image0, image1, models, device):
    ...
    for batch_idx in range(batch_size):
        with torch.inference_mode():
            ...
            # Extract features with SIFT + AffNet descriptor
            lafs1, resps1, descs1 = local_feature(image0_gray[batch_idx : batch_idx + 1])
            lafs2, resps2, descs2 = local_feature(image1_gray[batch_idx : batch_idx + 1])
        ...
        if idxs.shape[0] > 0:
            centers1 = KF.get_laf_center(lafs1).view(-1, 2)[idxs[:, 0]]
            centers2 = KF.get_laf_center(lafs2).view(-1, 2)[idxs[:, 1]]
