...
# -*- coding: utf-8 -*-
"""
SIFT + HardNet Evaluation Script

This script evaluates SIFT feature detection with HardNet descriptors and AdaLAM matching
...
def initialize_models(device, max_epipolar_distance=1.0):
    """Initialize SIFT detector, HardNet descriptor, AdaLAM matcher, and RANSAC."""
    # Create OpenCV SIFT detector with Kornia wrapper
    sift_detector = OpenCVDetectorKornia(cv2.SIFT_create(8000))
    
    # Create HardNet descriptor
    descriptor = KF.HardNet(pretrained=True).eval().to(device)
...
def extract_and_match_features(image0, image1, models, device):
    """
    Extract and match features using SIFT with HardNet descriptors and AdaLAM.
    ...
    for batch_idx in range(batch_size):
        with torch.inference_mode():
            ...
            # Extract features with SIFT + HardNet descriptor
