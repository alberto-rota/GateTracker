...
def get_matching_points_with_patches(
    ...
    if embedding_mask is not None:
        # [B, C, H_p, W_p] or [B, C, N_mask]; resize spatial grid to sqrt(N)×sqrt(N) if needed
        # (e.g. mask on raw H×W vs ViT tokens after backbone square resize).
        em = embedding_mask.mean(dim=1)
        g = int(N**0.5)
        if em.dim() == 3:
            if em.shape[1] * em.shape[2] != N:
                em = F.interpolate(
                    em.unsqueeze(1),
                    size=(g, g),
                    mode="bilinear",
            ...
            if em.shape[1] != N:
                n_mask = em.shape[1]
                hn = int(round(n_mask**0.5))
                if hn * hn != n_mask:
                    raise RuntimeError(
                        f"embedding_mask length {n_mask} is not a square grid; cannot align to N={N}"
                    )
                em = (
                    F.interpolate(
                        em.view(B, 1, hn, hn),
